...
export const ContagemEstoque: React.FC<ContagemEstoqueProps> = ({
  empresaSelecionada,
  onFechar,
}) => {
  ...
  const [resultadoDivergencias, setResultadoDivergencias] = useState<
    Divergencia[] | null
  >(null);
  const [nomeEmpresaAtual, setNomeEmpresaAtual] =
  ...
  const carregarDadosIniciais = async () => {
    ...
    try {
        ...
        if (isAdminMaster) {
          ...
          if (Array.isArray(empresasData)) {
            const encontrada = empresasData.find(
              (e: Empresa) => e.id.toString() === empresaSelecionada,
            );
            setNomeEmpresaAtual(
              encontrada
      ...
      const query = empresaSelecionada
      ...
      if (Array.isArray(data)) {
        ...
        // Tentar carregar rascunho salvo no Banco de Dados
        try {
          const queryRascunho = empresaSelecionada
            ? `?empresa_id=${empresaSelecionada}`
            : "";
          const rascunhoRes = await apiRequest(
            `estoque/inventario/rascunho${queryRascunho}`,
          );
          if (
            rascunhoRes &&
            Array.isArray(rascunhoRes.itens) &&
            rascunhoRes.itens.length > 0
          ) {
            setLinhasContagem(rascunhoRes.itens);
            setCarregando(false);
            return;
          }
        } catch (e) {
        ...
        // Se não houver rascunho, inicia linhas vazias para cada produto
        const linhasIniciais: ItemContadoLinha[] = data.map(
          (prod: ProdutoInventario) => ({
  ...
  const salvarRascunhoNoServidor = async (novasLinhas: ItemContadoLinha[]) => {
    try {
      await apiRequest("estoque/inventario/rascunho", {
        method: "POST",
        body: JSON.stringify({
          itens: novasLinhas,
          empresaId: empresaSelecionada ? Number(empresaSelecionada) : 1,
        }),
      });
    } catch (err) {
  ...
  const handleLinhaChange = (
    ...
    setLinhasContagem((prev) => {
      ...
      salvarRascunhoNoServidor(novasLinhas);
      return novasLinhas;
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const itensContados = linhasContagem
      .filter(
        (item) => item.lote.trim() !== "" || item.quantidadeContada !== "",
      )
      .map((item) => ({
    ...
    if (
      !confirm(
        "Deseja realmente finalizar e processar esta contagem de estoque? O relatório será gerado e os dados temporários serão limpos.",
      )
    )
      return;
    ...
    try {
      ...
      // Apagar o rascunho do banco de dados após sucesso
      const queryRascunho = empresaSelecionada
        ? `?empresa_id=${empresaSelecionada}`
        : "";
      await apiRequest(`estoque/inventario/rascunho${queryRascunho}`, {
        method: "DELETE",
      });
      ...
      // Mesmo se houver falha de rota na API mas gerarmos em fallback, limpamos o rascunho do servidor
      const queryRascunho = empresaSelecionada
        ? `?empresa_id=${empresaSelecionada}`
        : "";
      await apiRequest(`estoque/inventario/rascunho${queryRascunho}`, {
      ...
      const divergenciasCalculadas: Divergencia[] = itensContados.map(
        (item) => {
          const prod = produtos.find((p) => p.id === item.estoqueId);
          const qtdSistema = Number(
            prod?.quantidade ?? prod?.saldo ?? prod?.qtdSistema ?? 0,
          );
    ...
    return (
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 max-w-4xl w-full p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-200 print:border-black">
            <div className="flex items-center space-x-3">
              <CheckCircle2 className="h-7 w-7 text-emerald-600 shrink-0 print:text-black" />
              <div>
                <h2 className="text-lg sm:text-xl font-bold text-gray-800">
                  Relatório de Divergências
                </h2>
                <p className="text-xs text-gray-500">
                  Comparativo finalizado ({nomeEmpresaAtual}) - Data:{" "}
            ...
            <div className="flex items-center gap-2 print:hidden">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl transition text-sm font-medium"
              >
          ...
          <div className="overflow-x-auto mb-6">
            <table className="w-full text-left border-collapse text-xs sm:text-sm">
              <thead>
                <tr className="bg-gray-50 text-gray-700 uppercase tracking-wider border-b border-gray-200 print:bg-gray-200 print:text-black">
          ...
          <div className="flex justify-end print:hidden">
            <button
              onClick={onFechar}
              className="px-5 py-2 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition font-medium text-sm"
            >
              Fechar Relatório
            </button>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 max-w-5xl w-full p-4 sm:p-6 max-h-[94vh] flex flex-col">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                <Clock className="h-3 w-3 text-emerald-600 shrink-0" /> Salvo na
                nuvem automaticamente. Pode pausar quando quiser.
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto my-2 pr-1">
          {carregando ? (
            <div className="text-center py-16 text-gray-500 text-sm">
              A carregar produtos do estoque...
            </div>
          ) : produtos.length === 0 ? (
            <div className="text-center py-16 text-gray-500 text-sm">
            ...
            <>
              <div className="block md:hidden space-y-4">
                {produtos.map((prod) => {
                  ...
                  const linhasDoProduto = linhasContagem.filter(
                    (l) => l.estoqueId === prod.id,
                  );

                  return (
                    <div
                      key={prod.id}
                      className="bg-gray-50 border border-emerald-100 rounded-xl p-3.5 shadow-sm space-y-3"
                    >
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Embalagem: {prod.unidade || "-"}{" "}
                          {prod.embalagem ? `(${prod.embalagem})` : ""}
                        </p>
                      </div>
                      ...
                      <div className="space-y-2 pt-1 border-t border-gray-200">
                        {linhasDoProduto.map((linha, index) => (
                          <div
                            key={linha.idUnico}
                            className="bg-white p-2.5 rounded-lg border border-gray-200 space-y-2"
                          >
                          ...
                          {linhasDoProduto.map((linha, index) => (
                            ...
                            >
                              {index === 0 ? (
                                <>
                                  <td
                                    className="py-3 px-4 font-medium text-gray-900 align-top"
                                    rowSpan={linhasDoProduto.length}
                                  >
                                    {nomeExibicao}
                                  </td>
                                  <td
        ...
        <div className="pt-3 mt-2 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          >
            ...
            <span>
              {enviando ? "A processar..." : "Finalizar e Gerar Relatório"}
            </span>
          </button>
        </div>
      </div>
