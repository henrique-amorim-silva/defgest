...
export const Estoque: React.FC = () => {
  ...
  const [filtroValidade, setFiltroValidade] = useState<'todos' | 'validos' | 'vencidos'>('todos');
  const [ordenacao, setOrdenacao] = useState<'nome-asc' | 'qtd-desc' | 'qtd-asc'>('nome-asc');
  ...
  const resumoConsolidado = estoque.reduce((acc, item) => {
    ...
    acc[nome].quantidadeTotal += Number(item.quantidadeAtual);
    ...
    return acc;
  }, {} as Record<string, { nomeProduto: string; quantidadeTotal: number; unidade: string; totalLotes: number; lotesDetalhados: Array<{ lote: string; quantidade: number; validade: string; vencido: boolean }> }>);

  const listaConsolidada = Object.values(resumoConsolidado).filter((prod) =>
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden p-6 border border-emerald-100">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 border-b pb-4 gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Gerencie lotes, validades e verifique totais para tomada de decisão de compra.</p>
            </div>
          </div>
          ...
          <div className="flex bg-gray-100 p-1 rounded-lg border">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-semibold rounded-md transition ${
                abaVisualizacao === 'lotes' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-600 hover:text-gray-900'
              }`}
            >
              Visão por Lotes
            </button>
            <button
              ...
              className={`px-3 py-1.5 text-xs font-semibold rounded-md transition flex items-center space-x-1 ${
                abaVisualizacao === 'consolidado' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-600 hover:text-gray-900'
              }`}
            >
        ...
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mb-6 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Filtrar Produto</label>
            ...
            {abaVisualizacao === 'lotes' && (
              <>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Filtrar Lote</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Filtrar Nota Fiscal</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Status Validade</label>
                  <select
                    value={filtroValidade}
                    onChange={(e) => setFiltroValidade(e.target.value as 'todos' | 'validos' | 'vencidos')}
            ...
            <div>
              ...
              <select
                value={ordenacao}
                onChange={(e) => setOrdenacao(e.target.value as 'nome-asc' | 'qtd-desc' | 'qtd-asc')}
                className="w-full px-3 py-1.5 border border-gray-300 rounded-md text-xs bg-white focus:ring-2 focus:ring-emerald-500"
              >
        ...
        {abaVisualizacao === 'consolidado' ? (
          <div>
            <div className="mb-4 bg-emerald-50 border border-emerald-100 p-3 rounded-lg flex items-center justify-between">
              <p className="text-xs text-emerald-800">
                💡 <strong>Visão para Compras:</strong> Aqui você visualiza o <strong>total geral acumulado</strong> de cada produto somando todos os lotes. Use esta tela para saber exatamente se o volume atual é suficiente ou se precisa comprar mais.
              </p>
            </div>
            ...
            {listaConsolidada.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 font-medium">Nenhum produto consolidado encontrado.</p>
              </div>
            ) : (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50 text-xs text-gray-600 uppercase">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-center">Quantidade Total em Estoque</th>
                      <th className="px-6 py-3 text-center">Qtd. Lotes Ativos</th>
                      <th className="px-6 py-3 text-left">Detalhamento dos Lotes</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200 text-sm">
                    {listaConsolidada.map((item) => (
                      <tr key={item.nomeProduto} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-1 text-xs">
                            {item.lotesDetalhados.map((l, idx) => (
                              <div key={idx} className="flex items-center space-x-2 bg-gray-50 px-2 py-1 rounded border">
                                <span className="font-mono font-bold text-gray-700">Lote: {l.lote}</span>
                                <span className="text-emerald-700 font-semibold">({l.quantidade} {item.unidade})</span>
                                <span className="text-gray-400">| Val: {l.validade}</span>
                                {l.vencido ? (
                                  ...
                                  <span className="text-emerald-600 flex items-center space-x-0.5">
                                    <CheckCircle className="h-3 w-3" /> <span>No prazo</span>
                                  </span>
                                )}
                              </div>
                            ))}
          ...
          <div>
            {estoqueOrdenado.length === 0 ? (
              <div className="text-center py-12">
                <Layers className="mx-auto h-12 w-12 text-gray-300 mb-3" />
                <p className="text-gray-500 font-medium">Nenhum registro encontrado com os filtros atuais.</p>
              </div>
            ) : (
              <div className="overflow-x-auto border rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tr key={item.id} className="hover:bg-gray-50 transition">
                          ...
                          <td className="px-6 py-4 text-gray-600 text-xs">
                            <div className="flex items-center space-x-1.5">
                              ...
                              <span className={vencido ? "text-red-600 font-bold" : ""}>{item.dataValidade}</span>
                              {vencido && (
                                <span className="bg-red-100 text-red-700 px-1.5 py-0.5 rounded text-[10px] font-bold">VENCIDO</span>
                              )}
                            </div>
                          </td>
